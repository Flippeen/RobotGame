...
public class GridManager : MonoBehaviour
{
    ...
    {
        try{
            if (grid[nextPos.x, nextPos.y, nextPos.z]?.GetComponent<IInteractable>() != null)
            {
                grid[nextPos.x, nextPos.y, nextPos.z].GetComponent<IInteractable>().UseInteractable(grid[startPos.x, startPos.y, startPos.z].GetComponent<PlayerMovement>());
    ...
    public GridObject GetGridObjectOnPosition(Vector3Int positionofGO)
    {
        return grid[positionofGO.x, positionofGO.y, positionofGO.z];
    }
